<template>
  <div class="relative" ref="pickerRef">
    ...
    >
      ...
      <span class="font-semibold text-gray-700 dark:text-gray-300">{{ displayText }}</span>
    ...
    <!-- 弹出层 -->
    <div v-if="isOpen" class="absolute right-0 mt-2 bg-white dark:bg-gray-800 rounded-2xl shadow-2xl dark:shadow-gray-700/30 border border-gray-200 dark:border-gray-700 z-50 overflow-hidden" :class="showCustomPicker ? 'w-[640px]' : 'w-[380px]'">
      <!-- 当前选择提示 -->
      <div class="px-6 py-3 bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-900/30 dark:to-purple-900/30 border-b border-gray-200 dark:border-gray-700">
        <p class="text-sm text-gray-600 dark:text-gray-300">
          <span class="font-medium">当前：</span>
          <span class="text-indigo-600 dark:text-indigo-400 font-semibold">{{ currentRangeText }}</span>
        </p>
      </div>
      ...
      <div class="p-6">
        <!-- 快捷选项 -->
        <div class="mb-4">
          <h3 class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-3">快捷选项</h3>
          <div class="grid grid-cols-2 gap-2">
            <button
              ...
              :class="activeShortcut === shortcut.value
                ...
                : 'bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-300 border-gray-300 dark:border-gray-600 hover:border-indigo-400 dark:hover:border-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-900/30'"
            >
              {{ shortcut.label }}
            </button>
          </div>
        ...
        <div v-if="!showCustomPicker" class="mb-4">
          <h3 class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-3">选择月份</h3>

          <!-- 年份选择 -->
          <div class="flex items-center justify-between mb-3">
            <button @click="prevYear" class="p-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg text-gray-700 dark:text-gray-300">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
              </svg>
            </button>
            <span class="text-base font-bold text-gray-900 dark:text-white">{{ currentYear }}年</span>
            <button @click="nextYear" class="p-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg text-gray-700 dark:text-gray-300">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <button
              ...
              :class="isMonthDisabled(month)
                ...
                : isMonthSelected(month)
                  ? 'bg-indigo-600 text-white border-indigo-600 shadow-md'
                  : 'bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-300 border-gray-200 dark:border-gray-600 hover:border-indigo-400 dark:hover:border-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-900/30'"
            >
              {{ month }}月
            </button>
          </div>
        ...
        <div v-if="showCustomPicker">
          <h3 class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-3">自定义范围</h3>
          ...
          <div class="flex gap-4 mb-4">
            ...
            <div class="flex-1">
              <div class="text-center mb-3">
                <div class="flex items-center justify-between mb-2">
                  <button @click="prevMonth('start')" class="p-1 hover:bg-gray-100 dark:hover:bg-gray-700 rounded text-gray-700 dark:text-gray-300">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
                    </svg>
                  </button>
                  <span class="text-sm font-semibold text-gray-900 dark:text-white">{{ formatMonthYear(startCalendarMonth) }}</span>
                  <button @click="nextMonth('start')" class="p-1 hover:bg-gray-100 dark:hover:bg-gray-700 rounded text-gray-700 dark:text-gray-300">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
                    </svg>
                  </button>
            ...
            <div class="flex-1">
              <div class="text-center mb-3">
                <div class="flex items-center justify-between mb-2">
                  <button @click="prevMonth('end')" class="p-1 hover:bg-gray-100 dark:hover:bg-gray-700 rounded text-gray-700 dark:text-gray-300">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
                    </svg>
                  </button>
                  <span class="text-sm font-semibold text-gray-900 dark:text-white">{{ formatMonthYear(endCalendarMonth) }}</span>
                  <button @click="nextMonth('end')" class="p-1 hover:bg-gray-100 dark:hover:bg-gray-700 rounded text-gray-700 dark:text-gray-300">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
                    </svg>
                  </button>
          ...
          <!-- 验证提示 -->
          <div v-if="validationError" class="mb-3 p-3 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-lg">
        ...
        <div class="flex gap-3">
          <button
            ...
            class="flex-1 px-4 py-2.5 text-sm font-medium text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/30 border border-indigo-200 dark:border-indigo-800 rounded-lg hover:bg-indigo-100 dark:hover:bg-indigo-900/50 transition-all"
          >
            自定义范围
          </button>
          <template v-else>
            <button
              ...
              class="flex-1 px-4 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-600 transition-all"
            >
              取消
            </button>
            <button
              ...
              :class="isCustomRangeValid ? 'bg-indigo-600 hover:bg-indigo-700' : 'bg-gray-400 dark:bg-gray-600'"
            >
              应用
            </button>
          </template>
...
function getDateClass(dateObj, type) {
  ...
  if (dateStr === new Date().toDateString()) {
    return 'border border-indigo-400 dark:border-indigo-500 text-indigo-600 dark:text-indigo-400 font-medium'
  }
  ...
  return 'text-gray-700 dark:text-gray-300 hover:bg-indigo-50 dark:hover:bg-indigo-900/30'
}
